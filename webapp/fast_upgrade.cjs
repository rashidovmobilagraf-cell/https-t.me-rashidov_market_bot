...
async function run() {
  let client = null;
  console.log('Attempting parallel connections...');
  
  try {
      client = await Promise.any(regions.map(host => tryConnect(host)));
      console.log(`Successfully connected to a region!`);
  } catch (e) {
      console.error('Could not connect to any Supabase region.', e);
      return;
  }
